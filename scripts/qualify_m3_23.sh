...
WORK="$BUILD/m3_23-runtime"; SRC="$WORK/Musashi"; BIN="$WORK/librerom-m3_23-runtime"; EVIDENCE="$WORK/runtime.txt"
mkdir -p "$BUILD" "$WORK"
...
make -C "$SRC" -s all
gcc -std=c11 -Wall -Wextra -Werror -O2 -I"$SRC" "$ROOT/tests/m3_23_runtime.c" "$SRC/m68kcpu.o" "$SRC/m68kops.o" "$SRC/softfloat/softfloat.o" -lm -o "$BIN"
"$BIN" "$ROM" | tee "$EVIDENCE"
{ echo "musashi_commit=$MUSASHI_COMMIT"; sha256sum "$ROM"; } >> "$EVIDENCE"
echo "LibreROM M3.23 build/static/inherited-runtime qualification: PASS"
