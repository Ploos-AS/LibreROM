...
#define MARKER_VALUE 0x4c524d36u
#define EXC_VALUE 0x45584336u
#define PRE_VALUE 0x50524536u
...
static int load_rom(const char *path)
{
    ...
    if (got != sizeof(rom) || fgetc(fp) != EOF) {
        fprintf(stderr, "M2.6 runtime FAIL: ROM must be exactly %u bytes (got %zu)\n", ROM_SIZE, got);
        fclose(fp);
        return 1;
    }
    fclose(fp);
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        fprintf(stderr, "usage: %s <librom-m2.6-macplus.bin>\n", argv[0]);
        return 2;
    }
    if (load_rom(argv[1]))
        return 1;
    ...
    if (sp != RAM_SIZE || pc < ROM_BASE || pc >= ROM_BASE + ROM_SIZE) {
        fprintf(stderr, "M2.6 runtime FAIL: reset SP=%08x PC=%08x\n", sp, pc);
        return 1;
    }

    cycles = m68k_execute(14000);
    pc = m68k_get_reg(NULL, M68K_REG_PC);
    sr = m68k_get_reg(NULL, M68K_REG_SR);
    reset_pc = ram32(4);
    default_vector = ram32(32u * 4u);

    printf("LibreROM M2.6 runtime evidence\n");
    ...
    printf("marker=%08x exception_marker=%08x pre_marker=%08x\n",
           ram32(MARKER_ADDR), ram32(EXC_ADDR), ram32(PRE_ADDR));
    printf("ram_top_probe_restored=%08x\n", ram32(RAM_TOP_PROBE));

    if (!vectors_ready_before_overlay_off) {
        fprintf(stderr, "M2.6 runtime FAIL: vector table was not complete before overlay-off\n");
        return 1;
    }
    if (overlay || (via_ddra & OVERLAY_BIT) == 0 || (via_ora & OVERLAY_BIT) != 0) {
        fprintf(stderr, "M2.6 runtime FAIL: overlay state incorrect\n");
        return 1;
    }
    if (ram32(0) != RAM_SIZE || reset_pc < ROM_BASE || reset_pc >= ROM_BASE + ROM_SIZE) {
        fprintf(stderr, "M2.6 runtime FAIL: RAM reset vectors invalid\n");
        return 1;
    }
    if (default_vector < ROM_BASE || default_vector >= ROM_BASE + ROM_SIZE) {
        fprintf(stderr, "M2.6 runtime FAIL: exception vector 32 invalid\n");
        return 1;
    }
    if (ram32(MARKER_ADDR) != MARKER_VALUE || ram32(EXC_ADDR) != EXC_VALUE ||
        ram32(PRE_ADDR) != PRE_VALUE) {
        fprintf(stderr, "M2.6 runtime FAIL: hand-off markers missing\n");
        return 1;
    }
    if (ram32(RAM_TOP_PROBE) != 0) {
        fprintf(stderr, "M2.6 runtime FAIL: RAM probe was not restored\n");
        return 1;
    }
    if (pc < ROM_BASE || pc >= ROM_BASE + ROM_SIZE) {
        fprintf(stderr, "M2.6 runtime FAIL: execution left ROM window\n");
        return 1;
    }

    puts("LibreROM M2.6 runtime qualification: PASS");
    return 0;
}
