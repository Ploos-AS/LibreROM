...
BUILD="$ROOT/build"
OBJ="$BUILD/m3_2-reset.o"
ELF="$BUILD/librom-m3.2-macplus.elf"
ROM="$BUILD/librom-m3.2-macplus.bin"
MAP="$BUILD/librom-m3.2-macplus.map"
MUSASHI_COMMIT=313ebf1bd9f4d0d93341eb5ce21fd8a119e9dbdd
WORK="$BUILD/m3_2-runtime"
SRC="$WORK/Musashi"
BIN="$WORK/librerom-m3_2-runtime"
EVIDENCE="$WORK/runtime.txt"

mkdir -p "$BUILD"
python3 "$ROOT/scripts/check_m3_2.py"
"${CROSS}as" -m68000 -o "$OBJ" "$ROOT/src/platform/macplus/reset_m3_2.S"
"${CROSS}ld" -T "$ROOT/linker/m3_2.ld" -Map="$MAP" -o "$ELF" "$OBJ"
