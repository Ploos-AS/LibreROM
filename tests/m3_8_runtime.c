...
static int load(const char *p) {
    FILE *f=fopen(p,"rb"); size_t n;
    if(!f){perror(p);return 1;} n=fread(rom,1,sizeof rom,f); fclose(f);
    if(n!=sizeof rom){fprintf(stderr,"M3.8 FAIL: ROM size %zu\n",n);return 1;} return 0;
}
int main(int argc,char **argv){
    ...
    while(cycles<700000u && r32(0x420)!=0x4f4b3338u){int n=m68k_execute(64);cycles+=(unsigned)(n>0?n:64);}
    pc=m68k_get_reg(NULL,M68K_REG_PC);
    printf("LibreROM M3.8 runtime evidence\ncycles=%u\npc=%08x\noverlay=%d\n",cycles,pc,overlay);
    printf("heap_next=%08x mem_err=%04x master0=%08x\n",r32(0x440),r16(0x220),r32(0x7000));
    printf("hrec0 handle=%08x data=%08x logical=%08x extent=%08x active=%08x\n",
           r32(0x500),r32(0x504),r32(0x508),r32(0x50c),r32(0x510));
    if(overlay || (via_ddra & OVERLAY_BIT)==0 || (via_ora & OVERLAY_BIT)!=0) return 1;
    if(r32(0x400)!=0x4d333830u || r32(0x404)!=0x4e48384fu || r32(0x40c)!=0x4748384fu ||
       r32(0x410)!=0x524c384fu || r32(0x414)!=0x4752384fu || r32(0x418)!=0x5348384fu ||
       r32(0x41c)!=0x575a3838u || r32(0x420)!=0x4f4b3338u) return 1;
    if(r32(0x440)!=0x00010074u || r16(0x220)!=(uint16_t)0xff91u) return 1;
    if(r32(0x500)!=0x00007000u || r32(0x504)!=0x00010042u || r32(0x508)!=0x10u || r32(0x50c)!=0x32u || r32(0x510)!=0u) return 1;
    if(r32(0x7000)!=0u) return 1;
    if(r32(0x10042)!=0x48443841u || r32(0x10046)!=0x48443842u) return 1;
    if(r32(0x10022)!=0x424c4b38u) return 1;
    if(pc<ROM_BASE || pc>=ROM_BASE+ROM_SIZE) return 1;
    puts("LibreROM M3.8 GetHandleSize/SetHandleSize relocation qualification: PASS");
    return 0;
}
