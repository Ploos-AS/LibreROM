...
static int load(const char *p) {
    ...
    if(n!=sizeof rom){fprintf(stderr,"M3.6 FAIL: ROM size %zu\n",n);return 1;} return 0;
}
int main(int argc,char **argv){
    ...
    while(cycles<500000u && r32(0x420)!=0x4f4b3336u){int n=m68k_execute(64);cycles+=(unsigned)(n>0?n:64);}
    pc=m68k_get_reg(NULL,M68K_REG_PC);
    printf("LibreROM M3.6 runtime evidence\ncycles=%u\npc=%08x\noverlay=%d\n",cycles,pc,overlay);
    printf("heap_next=%08x mem_err=%04x\n",r32(0x440),r16(0x220));
    printf("slot0_ptr=%08x logical=%08x extent=%08x active=%08x\n",r32(0x480),r32(0x484),r32(0x488),r32(0x48c));
    if(overlay || (via_ddra & OVERLAY_BIT)==0 || (via_ora & OVERLAY_BIT)!=0) return 1;
    if(r32(0x400)!=0x4d333630u || r32(0x404)!=0x4e50364fu || r32(0x40c)!=0x4750364fu ||
       r32(0x410)!=0x53503653u || r32(0x414)!=0x53503647u || r32(0x418)!=0x4d463636u ||
       r32(0x41c)!=0x575a3636u || r32(0x420)!=0x4f4b3336u) return 1;
    if(r32(0x440)!=0x00010032u || r16(0x220)!=(uint16_t)0xff91u) return 1;
    if(r32(0x480)!=0x00010000u || r32(0x484)!=0x31u || r32(0x488)!=0x32u || r32(0x48c)!=0u) return 1;
    if(pc<ROM_BASE || pc>=ROM_BASE+ROM_SIZE) return 1;
    puts("LibreROM M3.6 GetPtrSize/SetPtrSize runtime qualification: PASS");
    return 0;
}
