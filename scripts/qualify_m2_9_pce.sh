...
cat "$TRANSCRIPT"
grep -Eq '00000400.*4C 52 4D 39.*45 58 43 39.*50 52 45 39.*56 49 44 39' "$TRANSCRIPT" || exit 1
grep -Eq '00000410.*4B 42 44 39' "$TRANSCRIPT" || exit 1
grep -Eq '00000420.*49 4E 50 39' "$TRANSCRIPT" || exit 1
grep -Eq '000FA700.*AA 55 AA 55' "$TRANSCRIPT" || exit 1
grep -Eq '000F2700.*AA 55 AA 55' "$TRANSCRIPT" || exit 1
grep -Eq 'IER=00[[:space:]]+IRQ=0' "$TRANSCRIPT" || exit 1
{
