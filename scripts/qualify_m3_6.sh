...
BUILD="$ROOT/build"
OBJ="$BUILD/m3_6-reset.o"
ELF="$BUILD/librom-m3.6-macplus.elf"
ROM="$BUILD/librom-m3.6-macplus.bin"
MAP="$BUILD/librom-m3.6-macplus.map"
MUSASHI_COMMIT=313ebf1bd9f4d0d93341eb5ce21fd8a119e9dbdd
WORK="$BUILD/m3_6-runtime"
SRC="$WORK/Musashi"
BIN="$WORK/librerom-m3_6-runtime"
EVIDENCE="$WORK/runtime.txt"

mkdir -p "$BUILD"
python3 "$ROOT/scripts/check_m3_6.py"
"${CROSS}as" -m68000 -o "$OBJ" "$ROOT/src/platform/macplus/reset_m3_6.S"
"${CROSS}ld" -T "$ROOT/linker/m3_6.ld" -Map="$MAP" -o "$ELF" "$OBJ"
