...
ROM=${1:-"$ROOT/build/librom-m2.12-macplus.bin"}
ELF=${2:-"$ROOT/build/librom-m2.12-macplus.elf"}
PCE_COMMIT=371414f8f41ae02e9ce36004ba7b076fdd3abe63
WORK="$ROOT/build/m2_12-pce"
SRC="$WORK/PCE"
...
STOP_HEX=$(${CROSS:-m68k-linux-gnu-}nm -n "$ELF" | awk '$3 == "_m2_12_stop" {print $1; exit}')
[ -n "$STOP_HEX" ] || exit 1
...
# Project-owned sector 0 payload. The first bytes are deliberately simple so
# firmware can prove that decoded payload, not merely raw GCR, reached RAM.
python3 - "$PAYLOAD" <<'PY'
from pathlib import Path
import sys
p = Path(sys.argv[1])
buf = bytearray(512)
marker = b"LR12BOOT-LIBREROM-SECTOR0\0"
buf[:len(marker)] = marker
for i in range(len(marker), 512):
    buf[i] = (i * 37 + 0x12) & 0xff
p.write_bytes(buf)
PY
...
grep -Eq '00002000.*4C 52 31 32 42 4F 4F 54' "$TRANSCRIPT"

{
    ...
    echo "payload_marker=LR12BOOT"
    echo "success_markers=SEC2/CHK2"
    sha256sum "$ROM" "$DISK" "$PAYLOAD"
    echo "LibreROM M2.12 PCE qualification: PASS"
