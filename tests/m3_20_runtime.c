...
static uint32_t r32(uint32_t a){return ((uint32_t)ram[a]<<24)|((uint32_t)ram[a+1]<<16)|((uint32_t)ram[a+2]<<8)|ram[a+3];} static uint16_t r16(uint32_t a){return (uint16_t)(((uint16_t)ram[a]<<8)|ram[a+1]);}
int main(int argc,char**argv){if(argc!=2)return 2;FILE*f=fopen(argv[1],"rb");if(!f)return 1;size_t n=fread(rom,1,sizeof rom,f);fclose(f);if(n!=sizeof rom)return 1;m68k_init();m68k_set_cpu_type(M68K_CPU_TYPE_68000);m68k_pulse_reset();unsigned cycles=0;while(cycles<1400000u&&r32(0x424)!=0x4f4b3230u){int x=m68k_execute(64);cycles+=(unsigned)(x>0?x:64);}printf("LibreROM M3.20 runtime evidence\ncycles=%u overlay=%d heap_next=%08x mem_err=%04x\n",cycles,overlay,r32(0x440),r16(0x220));printf("markers: HO20=%08x RU20=%08x SK20=%08x OK20=%08x\n",r32(0x410),r32(0x414),r32(0x418),r32(0x424));printf("master0=%08x reused_payload=%08x barrier_payload=%08x\n",r32(0x7000),r32(0x7fe80),r32(0x7fec0));if(overlay)return 1;if(r32(0x400)!=0x4d333230u||r32(0x410)!=0x484f3230u||r32(0x414)!=0x52553230u||r32(0x418)!=0x534b3230u||r32(0x424)!=0x4f4b3230u)return 1;if(r16(0x220)!=0u)return 1;if(r32(0x7000)!=0x00010000u)return 1;if(r32(0x7fe80)!=0x52553230u||r32(0x7fec0)!=0x42323020u)return 1;puts("LibreROM M3.20 interior Ptr reuse runtime qualification: PASS");return 0;}
