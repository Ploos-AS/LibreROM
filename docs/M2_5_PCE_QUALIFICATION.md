...
PCE is configured as a Macintosh Plus with an MC68000 and 1 MiB RAM. The LibreROM image is mapped at `$00400000` with a 128 KiB ROM block. The PCE monitor starts from reset and runs to LibreROM's `_m2_4_stop` label using a temporary execution breakpoint.

After reaching that point the monitor dumps eight bytes at `$00000400`. M2.5 passes only when the independent machine model reports both project-owned markers:
