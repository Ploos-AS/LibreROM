...
int main(int argc,char**argv){if(argc!=2)return 2;FILE*f=fopen(argv[1],"rb");if(!f)return 1;size_t n=fread(rom,1,sizeof rom,f);fclose(f);if(n!=sizeof rom)return 1;m68k_init();m68k_set_cpu_type(M68K_CPU_TYPE_68000);m68k_pulse_reset();unsigned cycles=0;while(cycles<500000u&&r32(0x41c)!=0x4f4b3339u){int x=m68k_execute(64);cycles+=(unsigned)(x>0?x:64);}printf("LibreROM M3.9 runtime evidence\ncycles=%u\noverlay=%d\nmem_err=%04x\n",cycles,overlay,r16(0x220));if(overlay)return 1;if(r32(0x400)!=0x4d333930u||r32(0x404)!=0x4c4b394fu||r32(0x40c)!=0x4c46394fu||r32(0x410)!=0x554c394fu||r32(0x414)!=0x5354394fu||r32(0x418)!=0x4e49394fu||r32(0x41c)!=0x4f4b3339u)return 1;if(r16(0x220)!=(uint16_t)0xff93u)return 1;puts("LibreROM M3.9 handle lock/state runtime qualification: PASS");return 0;}
